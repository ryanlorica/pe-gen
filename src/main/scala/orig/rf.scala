...
class PartialRFControl(c: PartialRFConfig) extends Bundle {
  val wEnable: Vec[Bool] = Vec(c.numInputs, Bool())
  val rEnable: Vec[Bool] = Vec(c.numOutputs, Bool())
  val wAddr: Option[Vec[UInt]] = if (!c.bpFirm) Some(Vec(c.numInputs, UInt(c.addrWidth.W))) else None
  val rAddr: Option[Vec[UInt]] = if (!c.bpFirm) Some(Vec(c.numOutputs, UInt(c.addrWidth.W))) else None
  // Each output can select which input of the opposite bus to bypass from
  val bpSel: Option[Vec[Vec[Bool]]] = if (c.bpSoft || c.bpFirm) Some(Vec(c.numOutputs, Vec(c.numCrossInputs, Bool()))) else None
}
...
class RFConfig(
  ...
  val bpNone: Boolean = bpType == "None"
  val bpSoft: Boolean = bpType == "Soft"
...
class RFControl(c: RFConfig) extends Bundle {
  ...
  override def cloneType: RFControl.this.type = new RFControl(c).asInstanceOf[this.type]

  val internal: Option[PartialRFControl] = if (c.numIntInputs > 0 || c.numIntOutputs > 0)
    Some(new PartialRFControl(c.intConfig)) else None
  val external: Option[PartialRFControl] = if (c.numExtInputs > 0 || c.numExtOutputs > 0)
